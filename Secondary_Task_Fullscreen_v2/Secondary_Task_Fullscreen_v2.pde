...
void draw () {
  ...
  delay(floor(scalar*(waitSpeeds[randWait[n]])));
...
void keyPressed(){
  ...
  if (key == CODED){
    ...
    // the up key makes shte speed faster
    else if (keyCode == UP){
      // but not faster than 0.1 * the original speed
      //(0 or negative values make the program crash)
      if (scalar > 0.1){
        scalar = scalar - 0.1;
      }
      else{
